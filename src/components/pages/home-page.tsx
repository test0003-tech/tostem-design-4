...
import {
  ...
  Award, Sparkles, VolumeX, CheckCircle,
} from 'lucide-react';
    ...
    <div className="pt-[88px] lg:pt-[132px]">
        ...
        <div className="max-w-[1400px] mx-auto px-4 lg:px-8">
          ...
          <motion.div variants={containerVariants} initial="hidden" whileInView="visible" viewport={{ once: true }} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {whyTostemItems.map((item: WhyTostemItem) => (
              <motion.div key={item.title} variants={itemVariants} className="bg-white rounded-xl p-6 shadow-sm hover:shadow-lg transition-all duration-300 group cursor-pointer" onClick={() => {
        ...
        <div className="max-w-[1400px] mx-auto px-4 lg:px-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {categoryData.map((cat, i) => (
              <motion.a key={cat.id} href={`#/${cat.id}`} onClick={(e) => { e.preventDefault(); navigateTo(cat.href); }} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: i * 0.1 }} className="group relative overflow-hidden rounded-xl shadow-sm hover:shadow-xl transition-all duration-300">
                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        {series.specifications.map((spec) => (
                          <div key={spec.label} className="bg-white rounded-lg p-3 border border-gray-100"><div className="text-xs text-tostem-text-muted">{spec.label}</div><div className="text-sm font-bold text-tostem-dark mt-1">{spec.value}</div></div>
                        ))}
                      </div>
                    </div>
                  </div>
        ...
        <div className="max-w-[1400px] mx-auto px-4 lg:px-8">
          ...
          <motion.div variants={containerVariants} initial="hidden" whileInView="visible" viewport={{ once: true }} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
            {designsData.map((design) => (
              <motion.div key={design.id} variants={itemVariants} className="group relative rounded-xl overflow-hidden cursor-pointer hover:shadow-lg transition-all duration-300" onClick={() => navigateTo(design.slug)}>
      ...
      <section className="py-16 md:py-24 bg-white">
        <div className="max-w-[1400px] mx-auto px-4 lg:px-8">
          ...
          <motion.div variants={containerVariants} initial="hidden" whileInView="visible" viewport={{ once: true }} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {blogPosts.map((post) => (
              <motion.article key={post.id} variants={itemVariants} className="group bg-white rounded-xl overflow-hidden shadow-sm hover:shadow-lg transition-all duration-300 border border-gray-100 cursor-pointer" onClick={() => navigateTo('blog')}>
                <div className="aspect-[16/9] relative overflow-hidden">
        ...
        <div className="max-w-[1400px] mx-auto px-4 lg:px-8">
          ...
          <motion.div variants={containerVariants} initial="hidden" whileInView="visible" viewport={{ once: true }} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
            {testimonials.map((testimonial) => (
              <motion.div key={testimonial.id} variants={itemVariants} className="bg-tostem-light-gray rounded-xl p-5 relative hover:shadow-md transition-shadow cursor-pointer" onClick={() => navigateTo('testimonials')}>
