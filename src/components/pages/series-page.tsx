  ...
  return (
    <div className="pt-[88px] lg:pt-[132px]">
        ...
        <div className="absolute inset-0 flex items-center">
          <div className="max-w-[1400px] mx-auto px-4 lg:px-8 w-full">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
              ...
              <h1 className="text-3xl md:text-5xl lg:text-6xl font-black text-white mb-4">{series.name}</h1>
      ...
      <section className="py-12 md:py-16">
        <div className="max-w-[1400px] mx-auto px-4 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div>
              <h2 className="text-2xl font-bold text-tostem-dark mb-6">Technical Specifications</h2>
              <div className="grid grid-cols-2 gap-4">
                {series.specifications.map((spec) => (
                  <motion.div key={spec.label} initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="bg-tostem-light-gray rounded-lg p-4">
                    <div className="text-xs text-tostem-text-muted uppercase tracking-wider">{spec.label}</div>
                    <div className="text-xl font-bold text-tostem-dark mt-1">{spec.value}</div>
                  </motion.div>
                ))}
              </div>
            </div>
      ...
      <section className="py-12 md:py-16 bg-tostem-light-gray">
        <div className="max-w-[1400px] mx-auto px-4 lg:px-8">
          <h2 className="text-2xl font-bold text-tostem-dark mb-8">Available Designs in {series.name}</h2>
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {[
              ...
              <a key={item.slug} href={`#/${item.slug}`} onClick={(e) => { e.preventDefault(); navigateTo(item.slug); }} className="bg-white rounded-lg p-4 text-center hover:shadow-md transition-shadow group">
                <div className="text-sm font-medium text-tostem-dark group-hover:text-tostem-blue transition-colors">{item.name}</div>
